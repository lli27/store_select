...
class KMedoids:
    ...
    def __init__(self, n_clusters=2, n_init=10, max_iter=100, random_state=None, init_size=None, n_jobs=4,
                 method="kmeans++"):
        """
        初始化参数
    ...
    def _init_centroids(self, X):
        ...
        if self.init_size is not None and self.init_size < n_samples:
            if self.init_size < self.n_clusters:
                logging.warning("init_size={} should be larger than n_clusters={}. Setting it to 3*n_clusters".format(
            ...
            X[np.ix_(init_indices, init_indices)]
